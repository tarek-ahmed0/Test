...
with st.expander(f":violet[Recommended Resturant] ( :gray[Suitable For] :blue[{food_type}]) 🍽", expanded = True):
    if not recomm_df.empty:
...
with st.expander(f":violet[Recommended Activities] ( :gray[Suitable For] :blue[{activity_type}]) 🏞", expanded = True):
    if not recomm_df.empty:
